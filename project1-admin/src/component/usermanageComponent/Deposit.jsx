...
const Deposit = ({id}) => {
  ...
  const getAlldashboardDetails = async () => {
    setLoading(true);
    let url = `${import.meta.env.VITE_API_URL}/api/transaction/get-all-deposit-single/${id}?page=${currentPage}&limit=${limit}&status=${transactionType}`;
    try {
