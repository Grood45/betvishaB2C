...
const { t, i18n } = useTranslation();
  ...
  const getAllUser = async () => {
    setLoading(true);
    let url = `${import.meta.env.VITE_API_URL}/api/admin/get-all-user-by-admin?page=${currentPage}&limit=10`;
    if (search) {
      url += `&search=${search}`;
    }
    if (userCategory) {
  ...
  return (
    <div>
        ...
        <FaUsers fontSize={"30px"} style={{color:iconColor}} />
        {t(`All`)} {t(`User`)}
      </p>
                ...
                <div className="rounded-[10px] flex justify-between  overflow-scroll items-center gap-3 bg-white p-2 w-[100%]">
                  ...
                  <div className="flex min-w-[180px] w-[100%] flex-col gap-1">
                    ...
                    <p className="text-xs flex  font-medium  ">
                      ...
                      <span className="text-[10px] font-semibold">({time24Hour})</span>
                    </p>
                  </div>
                  ...
                  <div className="flex min-w-[100px] w-[100%] flex-col gap-1">
                    ...
                    <p className="text-xs flex items-center justify-start rounded-sm font-semibold text-red-600 py-[2px] ">
                      {" "}
                      {/* <HiCurrencyDollar
                        fontSize={"15px"}
                        className="text-yellow-300"
                      /> */} 
                      {item?.bonus} {item?.currency}
                    </p>
                  </div>
                  ...
                  <p className={`font-semibold text-center text-white text-xs ${item.is_blocked?"bg-red-600":"bg-green-500"} p-1 rounded-md `}>{item.is_blocked?"Block":"Active"}</p>
                  </div>
