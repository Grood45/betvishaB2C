...
import { BsCalendarDateFill, BsEyeFill, BsEyeSlashFill, BsGridFill, BsListUl } from "react-icons/bs";
...
import { FaRegUser, FaUserCheck, FaUserSlash, FaSignal, FaUsers, FaPlusCircle, FaMinusCircle } from "react-icons/fa";
import { IoSettings, IoWallet } from "react-icons/io5";
import { MdOutlineDashboard } from "react-icons/md";
import { GiCard3Diamonds } from "react-icons/gi";
import { MdPendingActions } from "react-icons/md";
import { RiUserLine, RiLock2Line, RiDeleteBinLine, RiShieldUserLine, RiEditBoxLine, RiKey2Line, RiShieldCheckLine } from "react-icons/ri";
...
const UserManage = () => {
  ...
  const getAllUser = async () => {
    setLoading(true);
    let url = `${import.meta.env.VITE_API_URL}/api/admin/get-all-user-by-parent?page=${currentPage}&limit=${limit}`;

    // If filtering by "Live User" (handled client-side or separate param if API supported, but here handled by client-side filter post-fetch effectively effectively or by clearing param if not supported)
    // Actually, based on existing code, `userCategory` is passed as `category` parameter. 
    // If "Live User" isn't a backend category, we might need a workaround. 
    // However, usually these are backend filters. 
    // Let's assume for now we pass it and if backend doesn't handle it, we filter client side.
    // BUT the task says "logic to filter". The API call uses `category`. 
    // If I add "live_user" to category, it goes to backend. 
    // If the backend doesn't support it, I should filter `allUserData` after setting it. 
    // Uses `userCategory` state.

    // Let's modify the filtering logic. The user wants the filter to work.
    // If `userCategory` is "live_user", we might need to filter `allUserData` locally if the API doesn't support it.
    // OR we change the URL construction.

    // Changing approach: Fetch all then filter if "live_user", OR pass param.
    // I will try passing it as category first, but since I cannot see backend code, 
    // I will implement a client-side filter effect on the DISPLAY logic for immediate results, 
    // OR filter the results after fetching if the count is small.
    // Given pagination, client-side filtering page-by-page is bad. 
    // But for "Live User" specifically, the user likely wants to see ALL live users.

    // Let's stick to adding the filter option UI first, and assume we might need to filter the `allUserData` implementation-side 
    // if the backend `category=live_user` doesn't return what we want.
    // Wait, the previous steps showed `isUserOnline` function helper. 
    // I can use that to specific filter the LIST being mapped.
    ...
    try {
      ...
      if (userCategory === "live_user") {
        receivedData = receivedData.filter(item => isUserOnline(item?.updated_at));
      }
  ...
  return (
    <div className="px-2 lg:px-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-6">
        <div className="bg-white/40 backdrop-blur-md p-4 rounded-2xl border border-white/20 shadow-xl flex items-center gap-4 group hover:scale-[1.02] transition-all cursor-pointer" style={{ borderLeft: `4px solid ${iconColor}` }} onClick={() => setUserCategory("")}>
          <div className="h-12 w-12 rounded-xl flex items-center justify-center shadow-inner" style={{ backgroundColor: `${iconColor}20`, color: iconColor }}>
            <FaUsers fontSize="24px" />
          </div>
          <div>
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t(`Total Users`)}</p>
            <p className="text-xl font-black text-gray-800">{usersCount.totalUserCount}</p>
          </div>
        </div>

        <div className="bg-white/40 backdrop-blur-md p-4 rounded-2xl border border-white/20 shadow-xl flex items-center gap-4 group hover:scale-[1.02] transition-all cursor-pointer" style={{ borderLeft: `4px solid #10b981` }} onClick={() => setUserCategory("is_active")}>
          <div className="h-12 w-12 rounded-xl flex items-center justify-center bg-green-50 text-green-500 shadow-inner">
            <FaUserCheck fontSize="22px" />
          </div>
          <div>
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t(`Active`)}</p>
            <p className="text-xl font-black text-gray-800">{usersCount.activeUserCount || 0}</p>
          </div>
        </div>

        <div className="bg-white/40 backdrop-blur-md p-4 rounded-2xl border border-white/20 shadow-xl flex items-center gap-4 group hover:scale-[1.02] transition-all cursor-pointer" style={{ borderLeft: `4px solid #ef4444` }} onClick={() => setUserCategory("is_blocked")}>
          <div className="h-12 w-12 rounded-xl flex items-center justify-center bg-red-50 text-red-500 shadow-inner">
            <FaUserSlash fontSize="22px" />
          </div>
          <div>
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t(`Blocked`)}</p>
            <p className="text-xl font-black text-gray-800">{usersCount.blockUserCount || 0}</p>
          </div>
        </div>

        <div className="bg-white/40 backdrop-blur-md p-4 rounded-2xl border border-white/20 shadow-xl flex items-center gap-4 group hover:scale-[1.02] transition-all cursor-pointer relative overflow-hidden" style={{ borderLeft: `4px solid #f59e0b` }} onClick={() => setUserCategory("live_user")}>
          <div className="absolute top-0 right-0 h-16 w-16 bg-orange-500/5 rounded-full -mr-8 -mt-8 animate-pulse"></div>
          <div className="h-12 w-12 rounded-xl flex items-center justify-center bg-orange-50 text-orange-500 shadow-inner">
            <FaSignal fontSize="20px" className="animate-pulse" />
          </div>
          <div className="z-10">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t(`Live User`)}</p>
            <p className="text-xl font-black text-gray-800">{allUserData?.filter(u => isUserOnline(u?.updated_at))?.length || 0}</p>
          </div>
        </div>

        <div className="bg-gradient-to-br from-indigo-600 to-purple-700 p-4 rounded-2xl border border-white/20 shadow-2xl flex items-center gap-4 group hover:scale-[1.02] transition-all cursor-pointer">
          <div className="h-12 w-12 rounded-xl flex items-center justify-center bg-white/20 text-white shadow-inner backdrop-blur-sm">
            <FaWallet fontSize="22px" />
          </div>
          <div>
            <p className="text-xs font-bold text-white/70 uppercase tracking-wider">{t(`Total Wallet`)}</p>
            <p className="text-xl font-black text-white">
              {(allUserData?.reduce((acc, curr) => acc + (curr.amount || 0), 0) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </p>
          </div>
        </div>
      </div>
                    ...
                    >
                      ...
                      <div style={{ background: `linear-gradient(135deg, ${iconColor}, ${secondaryBg})` }} className="p-4 flex justify-between items-start relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-3 relative z-10">
                          <Avatar size="sm" name={item?.username} src="https://bit.ly/dan-abramov" border="2px solid white" />
