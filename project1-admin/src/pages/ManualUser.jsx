...
import { FaRegUser, FaUsers, FaPlusCircle, FaMinusCircle } from "react-icons/fa";
import { MdPendingActions } from "react-icons/md";
import { RiUserLine, RiDeleteBinLine, RiEditBoxLine, RiKey2Line, RiShieldCheckLine } from "react-icons/ri";
...
const ManualUser = () => {
    ...
    const getAllUser = async () => {
        setLoading(true);
        // Added creation_mode=manual to the query
        let url = `${import.meta.env.VITE_API_URL}/api/admin/get-all-user-by-parent?page=${currentPage}&limit=${limit}&creation_mode=manual`;
        ...
        try {
            ...
            if (timeFilter) {
                ...
                receivedData = receivedData.filter((item) => {
                    const joinedAt = moment(item.joined_at, "YYYY-MM-DD hh:mm:ss A");

                    if (timeFilter === "24h") {
                        return now.diff(joinedAt, 'hours') <= 24;
                    } else if (timeFilter === "1week") {
                        return now.diff(joinedAt, 'days') <= 7;
                    } else if (timeFilter === "1month") {
                        return now.diff(joinedAt, 'days') <= 30;
                    } else if (timeFilter === "custom") {
                        if (startDate && endDate) {
                            const start = moment(startDate).startOf('day');
                            const end = moment(endDate).endOf('day');
                            return joinedAt.isBetween(start, end, null, '[]');
                        }
                        return true;
                    }
                    return true;
    ...
    }, [currentPage, search, timeFilter, limit, startDate, endDate]);
    ...
    const profitLossData = async (userData) => {
        ...
        try {
            let response = await fetchGetRequest(url);
            setLoading1(false);
            if (response) setPlData(response);
        } catch (error) {
            setLoading1(false);
            console.log(error);
        }
    ...
    const fetchSportDetails = async (userData) => {
        ...
        try {
            const response = await fetchGetRequest(
                `${import.meta.env.VITE_API_URL}/api/sport/get-sport-balance-admin?username=${userData?.username}`
            );
    ...
    return (
        <div className="px-2 lg:px-0">
            <div className="flex flex-col items-end gap-3 md:flex-row justify-between px-2">
                {/* Adjusted container width to accommodate Custom Date Filter */}
                <div className="flex items-center md:w-[75%] gap-2 flex-wrap sm:flex-nowrap">
                    {/* Search */}
                    <div
                    ...
                    <select
                        ...
                        style={{ border: `1px solid ${border}`, backgroundColor: primaryBg }}
                        className={`p-[8px] w-[60%] rounded-[6px] text-xs md:text-sm font-semibold border outline-none`}
                        value={timeFilter}
                    >
                        <option value="24h">{t(`Last 24 Hours`)}</option>
                        <option value="1week">{t(`Last 1 Week`)}</option>
                        <option value="1month">{t(`Last 1 Month`)}</option>
                        <option value="custom">{t(`Custom Date`)}</option>
                    </select>

                    {/* Custom Date Inputs - Only show when 'Custom Date' is selected */}
                    {timeFilter === "custom" && (
                        <div className="flex items-center gap-1 w-full sm:w-auto">
                            <input
                                ...
                                className="p-[6px] border rounded text-xs outline-none"
                    ...
                    <button style={{ backgroundColor: bg }} className={`flex items-center justify-center p-[2px] gap-2 rounded-[6px] text-xs md:text-sm text-white font-bold w-[70px] min-w-[70px] h-[35px] hover:border-green-600 hover:border`}>
                        <img src={pdfIcon} alt="PDF" className="h-[25px] w-auto" />
                    </button>
                    <button style={{ backgroundColor: bg }} className={`flex items-center justify-center p-[2px] gap-2 rounded-[6px] text-xs md:text-sm text-white font-bold w-[70px] min-w-[70px] h-[35px] hover:border-green-600 hover:border`}>
                        <img src={excelIcon} alt="Excel" className="h-[25px] w-auto" />
                    </button>
            ...
            <div className="flex justify-between flex-col gap-4 sm:flex-row mt-6 pr-5 sm:items-center">
                <p style={{ color: iconColor }} className={`font-bold w-[100%] flex items-center gap-2 rounded-[6px] text-lg`}>
                    <IoIosPersonAdd style={{ color: iconColor }} fontSize={"30px"} />
                    {t(`Manual User`)}
                    <span className={`text-green-600`}>({allUserData?.length || 0})</span>
                </p>
            ...
            {/* Logic to Render Users (Same as NewUser.jsx) */}
                ...
                {allUserData && !loading && allUserData.length > 0 && (
                    viewMode === 'list' ? (
                        allUserData?.map((item) => {
                            ...
                            return (
                                <div key={item._id} style={{ border: `1px solid ${border}` }} className={`rounded-[10px] w-[100%]`}>
                                    <div className="rounded-[10px] flex justify-between overflow-scroll items-center gap-3 bg-white p-2 w-[100%]">
                                        <div className="flex items-center min-w-[150px] gap-1">
                                            ...
                                            <div className="flex flex-col items-start gap-1">
                                                ...
                                                <div className="flex gap-1">
                                                    <span style={{ backgroundColor: bg }} className={`text-[8px] font-medium px-1 py-[2px] rounded-[4px] text-white`}>
                                                        {item.role_type}
                                                    </span>
                                                    {/* Added Manual Badge for List View */}
                                        ...
                                        {/* Using the same list layout elements as NewUser */}
                                        <div className="flex min-w-[190px] w-[100%] flex-col gap-1">
                                            <p className="text-xs flex items-center gap-1 font-bold">
                                                <BsCalendarDateFill style={{ color: iconColor }} /> {t(`Joining`)} {t(`Date`)}
                                            </p>
                                            <p className="text-xs flex gap-1 font-medium">
                                                {formatDate(item?.joined_at.split(" ")[0])} <span className="text-[9px]">({time24Hour})</span>
                                            </p>
                                        </div>
                                        ...
                                        <div className="flex flex-col min-w-[140px] w-[100%] gap-1">
                                            <p className="text-xs flex items-center gap-1 font-bold">
                                                <FaWallet style={{ color: iconColor }} /> {t(`Balance`)}
                                            </p>
                                        ...
                                        {check && (
                                            <div className="flex min-w-[100px] flex-col gap-1">
                                                <p className="text-xs flex items-center gap-1 font-bold">
                                                    <MdPendingActions style={{ color: iconColor }} /> {t(`Status`)}
                                                </p>
                                                <Menu>
                                                    <MenuButton className={`${statuses[item._id] === "Block" ? "bg-red-600" : "bg-green-500"} px-2`} style={{ fontSize: "13px", borderRadius: "4px", color: "white" }}>
                                        ...
                                        {check && (
                                            <div className="flex items-center justify-end min-w-[170px] gap-[4px] ml-auto shrink-0">
                                                <Link to={`/usermanage/${item.username}`} style={{ border: `1px solid ${border}` }} className={`w-7 h-7 flex-shrink-0 flex items-center justify-center rounded-[6px] h-7 hover:bg-gray-100`}>
                                                    <FaRegUser style={{ color: iconColor }} fontSize={"14px"} />
                                                </Link>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 w-full">
                            {allUserData?.map((item) => (
                                <div key={item._id} className="rounded-xl overflow-hidden bg-white shadow-sm hover:translate-y-[-4px] transition-all duration-300 border border-gray-100 group relative">
                                    {/* Manual Badge Ribbon for Card View */}
                                    ...
                                    {/* Card Header */}
                                    <div style={{ background: `linear-gradient(135deg, ${iconColor}, ${primaryBg})` }} className="p-4 flex justify-between items-start relative overflow-hidden">
                                    ...
                                    {/* Card Body & Actions (Simplified for brevity, same structure as NewUser) */}
                                    <div className="p-4">
                                        ...
                                        {check && (
                                            <div className="grid grid-cols-6 gap-0 px-1 py-3 border-t border-gray-100 mt-2 items-start justify-items-center">
                                                ...
                                                <div className="flex flex-col items-center gap-1 cursor-pointer group w-full">
                                                    <ChangePassword id={item.user_id} type="user" userName={item?.username} icon={<div className="p-2 rounded-lg bg-gray-50 group-hover:bg-orange-50 group-hover:text-orange-600 transition-colors"><RiKey2Line size={16} /></div>} />
                                                    ...
                                                    <AddBalance
                                                        ...
                                                        customButton={
                                                            <div className="flex flex-col items-center gap-1 w-full h-full">
                                                                ...
                                                                <span className="text-[8px] font-black text-gray-400 group-hover:text-green-600 tracking-tighter text-center block w-full whitespace-nowrap text-center block w-full whitespace-nowrap">DEPOSIT</span>
                                                            </div>
                                                        }
                                                    />
                                                </div>
                                                    ...
                                                    <SubtractBalance
                                                        ...
                                                        customButton={
                                                            <div className="flex flex-col items-center gap-1 w-full h-full">
                                                                ...
                                                                <span className="text-[8px] font-black text-gray-400 group-hover:text-red-600 tracking-tighter text-center block w-full whitespace-nowrap text-center block w-full whitespace-nowrap">WITHDRAW</span>
                                                            </div>
                                                        }
                                                    />
                                                </div>
                                                ...
                                                {/* 5. DELETE */}
                                                <div className="flex flex-col items-center gap-1 cursor-pointer group w-full">
                                                    <DeleteUserAdmin type="user" name={item.username} id={item.user_id} onDeleteSuccess={deleteUserHandler} icon={<div className="p-2 rounded-lg bg-gray-50 group-hover:bg-red-50 group-hover:text-red-600 transition-colors"><RiDeleteBinLine size={16} /></div>} />
            ...
            <div className="flex justify-between items-center mt-4 w-full px-4 py-4 bg-gray-100/80 backdrop-blur-sm border-t border-gray-200">
                {allUserData?.length > 0 && (
                    <p className="text-sm font-bold text-gray-800">
                        {t(`Showing`)} 1 {t(`to`)} {allUserData.length} of {usersCount.totalUserCount} {t(`entries`)}
                    </p>
                )}
                {allUserData?.length > 0 && (
                    <div className="flex gap-4 items-center">
                        <button onClick={handlePrevPage} disabled={currentPage == 1} className="h-9 w-9 flex items-center justify-center bg-black text-white rounded-lg hover:bg-gray-800 disabled:bg-gray-300">{"<"}</button>
                        <div className="text-sm font-bold text-gray-800 flex gap-2 items-center bg-white px-4 py-2 rounded-lg border">
                            <span>{currentPage}</span> of <span>{pagination?.totalPages}</span>
                        </div>
                        <button onClick={handleNextPage} disabled={currentPage == pagination?.totalPages} className="h-9 w-9 flex items-center justify-center bg-black text-white rounded-lg hover:bg-gray-800 disabled:bg-gray-300">{">"}</button>
                    </div>
                )}
            </div>
