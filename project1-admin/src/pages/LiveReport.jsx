...
import { Progress, Radio, RadioGroup, Stack ,useToast,Badge} from '@chakra-ui/react';
...
const getCasinoBetHistory = async () => {
  setLoading(true);
  let url = `${import.meta.env.VITE_API_URL}/api/bet/get-all-bet?status=pending&page=${currentPage}&limit=${limit}`;
  ...
  <Stack spacing={4} wrap={"wrap"}  direction='row'>
    ...
    <Radio value='7'>Bookmaker </Radio>
      ...
      <p style={{color:iconColor}} className={`font-bold mt-6  w-[100%]    flex items-center gap-2 rounded-[6px]  text-lg`}>
        ...
        {t(`Casino`)} {t(`Live`)} {t(`Bet`)}  <span className={`text-green-600`}></span>
        ...
        <option value="20">20</option>
          ...
          <option value="1000">1000</option>
            ...
            placeholder={`${t(`search`)} ${t(`username`)}`}
                ...
                style={{border:`1px solid ${border}`}} className={`border pl-4 outline-none text-xs rounded-[4px] p-[8px]  `} />
                <div className="bg-white flex items-center cursor-pointer shadow-md gap-2 rounded-md px-4 p-1">
      <p className="text-[16px] font-semibold text-gray-800">{t(`Total`)} {t(`Running`)} {t(`Bet`)} {t(`Amount`)}:</p>
...
setCurrentPage(1)
      ...
      <p className="text-[16px] font-semibold text-gray-800">{t(`All`)} {t(`Running`)} {t(`Bet`)}:</p>
      <p className="text-lg font-bold text-blue-600">{betsCount?.pendingBet}</p>
    </div>
      ...
      <div>
        ...
        >
          ...
          <p style={{color:iconColor}} className={`font-semibold text-sm  pt-2 text-left`}>
  ...
  <table className={`w-full`}>
    <thead>
      <tr style={{ borderBottom: `1px solid ${border}` }} className={`text-center p-2 rounded-md h-[30px] text-[10px] font-bold`}>
        ...
        <th className="min-w-[120px] ">{t(`Provider`)} {t(`Name`)}</th>
                    <th className="min-w-[120px] ">{t(`Game`)} {t(`Name`)}</th>
        ...
        casinoBetData?.map((item) => {
          return (
            <tr key={item?._id} style={{ borderBottom: `1px solid ${border}` }} className={`text-center h-[60px] m-auto text-xs`}>
              ...
              <td className={`font-bold  ${item.Status=="running"?"text-orange-400":item.WinLoss !== "0" ? "text-green-600" : "text-red-600"}`}> {item.Status!=="running"&&(item.WinLoss !== "0" ? "+" : "-")}{item.Status=="running"?"0":item.WinLoss!=="0"?item.WinLoss?.toFixed(2):item.Amount?.toFixed(2)} {item?.currency}</td>
  ...
  {loading&&casinoBetData.length==0?
      ...
      {casinoBetData?.length>0&&    <p style={{color:iconColor}} className="text-xs font-semibold ">{t(`Showing`)} 1 {t(`to`)} {limit} {t(`of`)} 12 {t(`Entries`)}</p>}
