...
import { MdEdit, MdSportsEsports } from "react-icons/md";
...
const ManualDeposit = () => {
  ...
  const handleImageChange = (event,value) => {
    ...
    handleImageUpload(file,value);
  };

  const handleImageChange1 = (event,value) => {
    const file = event.target.files[0];
    handleImageUpload(file,value);
  };
  ...
  const deletePaymentGateway = async (id) => {
    try {
      const response = await sendDeleteRequest(
        `${
          import.meta.env.VITE_API_URL
        }/api/payment/delete-payment-method/${id}`
      );
  ...
  const getPaymentGateway = async () => {
    ...
    try {
      ...
      console.log(response, "deposit get data");
      setGetLoading(false);
    } catch (error) {
      console.error("Error uploading image:", error.message);
      toast({
  ...
  // image upload
  const handleImageUpload = async (file,value) => {
    if(value==1){
      setLoading(true);
    ...
    try {
      ...
      if (response.url) {
        ...
        if(value==1){
          ...
          setSelectedImage1(response.url);
          setLoading1(false);
        }
  ...
  const handleUpdateData = (data) => {
    ...
    setSelectedImage1(data?.qr_code)
  };
  ...
  return (
    <div className=" mt-8">
      ...
      {!show ? (
        <div>
          <div
            className={`flex justify-between mt-10    w-[90%]  m-auto items-center`}
          >
            <div
          ...
          <div className="p-4 mt-5 ml-10">
            {getLoading ? (
              <LoadingSpinner size="xl" color="green" thickness="4px" />
            ) : (
              ""
            )}
          </div>
        ...
        <div>
          ...
          <div className=" w-[95%] md:w-[85%] m-auto gap-4 flex flex-col md:flex-row justify-between">
            ...
            >
              ...
              {selectedImage ? (
                ...
                <div>
                   <p className="font-bold">Select getway logo</p>
                   <img
            ...
            >
              {selectedImage1 ? (
                ...
                <div>
                   <p className="font-bold">Upload getway Qrcode</p>
                   <img
                  src="https://www.freeiconspng.com/thumbs/no-image-icon/no-image-icon-6.png"
                  className="w-[140px]"
                />
          ...
          <div className=" w-[95%] md:w-[85%] mt-5 m-auto gap-4 flex flex-col md:flex-row justify-between">
            <div
              style={{ border: `1px solid ${border}` }}
              className={`w-[100%%] md:w-[85%]  bg-white  font-semibold rounded-[12px] h-[280px]`}
            >
