...
const WithDrawlManage = () => {
  ...
  const getWithdrawDetails = async () => {
    ...
    }/api/admin/get-all-withdraw-data-of-self?page=${currentPage}&limit=20`;
    if (transactionType) {
    ...
    <div className="lg:px-4">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  justify-between  gap-6 w-[100%]">
        ...
        <div className=" bg-white  p-3  flex flex-col justify-between z-[1] w-[100%] rounded-2xl h-[240px] top-7 left-1 md:left-2">
          <div className="w-[100%] flex justify-between">
            <div className="flex flex-col ">
              ...
              <p className="text-lg font-semibold ">{adminData?.amount?.toFixed(2)} {adminData?.currency}</p>
            </div>
            <svg
              width="116"
              height="71"
        ...
        <Barchart
             title={`${t(`Withdraw`)}`}
            type={"bar"}
            withdrawals={withdrawGraphData?.lastTwelveMonthsWithdrawAmountSelf}  
          />
        </div>
      </div>
...
export default WithDrawlManage;
