...
import { FaHistory, FaGlobe, FaClock, FaShieldAlt } from "react-icons/fa";
...
const LoginHistory = () => {
  ...
  const [role, setRole] = useState("alladmin");
  ...
  const getAllLoginHistory = async () => {
    ...
    let url = `${import.meta.env.VITE_API_URL
      }/api/login-history/get-history?page=${currentPage}&limit=${limit}&role=${role}`;
    if (search) {
      url += `&search=${search}`;
    }
    ...
    try {
      ...
      console.log(response, "all login history");
      setLoading(false);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white/40 backdrop-blur-md p-5 rounded-2xl border border-white/20 shadow-xl flex items-center gap-5 hover:scale-[1.02] transition-all cursor-pointer overflow-hidden relative group">
            ...
            <div className="z-10">
              ...
              <p className="text-lg font-black text-gray-800 italic uppercase">
                {loginHistoryData[0]?.updated_time?.split(" ")?.[0] || "No Records"}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col mt-4 md:flex-row items-center justify-between mb-6 gap-4">
        <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
          ...
          <select
            ...
            className="w-full md:w-48 p-3 outline-none rounded-xl border border-gray-200 bg-white/50 backdrop-blur-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all shadow-sm font-bold text-gray-600 cursor-pointer"
          >
            <option value="alladmin">{t(`All Roles`)} </option>
            {user.user?.role_type && (
              <option value={"owneradmin"}>{t("Owner Admin")}</option>
            )}
            {adminLayer?.slice(0, -1).map((ele) => (
              <option key={ele} value={ele}>{t(ele)}</option>
            ))}
          </select>
        </div>

        <div className="flex items-center gap-3 bg-white/50 backdrop-blur-sm p-2 rounded-xl border border-gray-200 shadow-sm ml-auto">
          ...
          <table className="min-w-full">
            <thead>
              <tr className="bg-gray-50/50">
                ...
                <th className="py-4 px-6 text-xs font-black text-gray-500 uppercase tracking-widest border-b border-gray-100 text-right">{t(`Action`)}</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={entry?._id} className="text-center">
                  ...
                  <td className="py-4 px-6 border-b border-gray-50 text-left">
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full bg-purple-50 border border-purple-100 flex items-center justify-center text-purple-600 font-bold text-xs shadow-sm">
                        {entry?.username?.charAt(0).toUpperCase()}
                      </div>
                      <span className="font-bold text-gray-800 tracking-tight italic">{entry?.username}</span>
                    </div>
                  </td>
                  <td className="py-4 px-6 border-b border-gray-50 text-left text-sm text-gray-500 font-medium tracking-tight">{entry?.email}</td>
                  <td className="py-4 px-6 border-b border-gray-50">
                    <span className="px-2.5 py-1 rounded-md bg-indigo-50 text-[10px] font-black uppercase text-indigo-500 border border-indigo-100">
                      {entry?.role}
                    </span>
                  </td>
                  <td className="py-4 px-6 border-b border-gray-50">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[10px] font-black text-gray-400 opacity-60 uppercase tracking-tighter italic">({entry?.created_time?.split(" ")?.[1]})</span>
                    </div>
                  </td>
                  <td className="py-4 px-6 border-b border-gray-50">
                    <div className="flex flex-col items-center">
                      <span className="text-sm font-black text-emerald-600 italic">{entry?.updated_time?.split(" ")?.[0]}</span>
                      <span className="text-[10px] font-black text-gray-400 opacity-60 uppercase tracking-tighter italic">({entry?.updated_time?.split(" ")?.[1]})</span>
                    </div>
                  </td>
      ...
      <div className="flex justify-between items-center mt-8 w-full px-6 py-5 bg-gray-100/80 backdrop-blur-sm border-t border-gray-200">
        {loginHistoryData?.length > 0 && (
          <p className="text-sm font-black text-gray-800 uppercase tracking-tighter">
            {t(`Showing`)} <span className="text-purple-600 italic">1</span> {t(`to`)} <span className="text-purple-600 italic">{limit}</span> {t(`of`)} <span className="text-purple-600 italic">{loginHistoryData?.length}</span> {t(`Entries`)}
          </p>
        )}
        {loginHistoryData && loginHistoryData?.length > 0 && (
          <div className="flex gap-4 items-center">
            ...
            <div className="text-sm font-black text-gray-800 flex gap-2 items-center bg-white px-5 py-2.5 rounded-xl border border-gray-200 shadow-sm">
              <span className="text-gray-400 font-black uppercase tracking-widest text-[10px]">{t(`Page`)}</span>
              <span className="text-purple-600 text-lg italic tracking-tighter">{currentPage}</span>
              <div className="h-4 w-[1px] bg-gray-200 mx-1"></div>
              <span className="text-gray-400 font-black uppercase tracking-widest text-[10px]">{t(`of`)}</span>
