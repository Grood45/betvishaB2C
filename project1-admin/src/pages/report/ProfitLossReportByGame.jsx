...
const ProfitLossByGame = () => {
  ...
  const getAllCasinoProvider = async () => {
    let url = `${
      import.meta.env.VITE_API_URL
    }/api/casinoprovider/get-provider?limit=200`;
  ...
  const generateTableHtml = (records) => {
    ...
    const tableRows = records.map((record,index) => {
      ...
      return `
        <tr class="text-xs">
          ...
          <td class="border px-4 py-2">${record?.GameTypeName?.slice(0,15) || "N/A"}</td>
          <td class="border px-4 py-2">${gameProviderName}</td>
          <td class="border px-4 py-2">${record.Username}</td>
          <td class="border px-4 py-2">${record.GamePeriodId}</td>
    ...
    const tableHtml = `
      <div>
        <p class="font-bold text-center text-lg my-4">Game Wise Report Card</p>
        <table class="w-full">
          <thead>
            <tr class="text-xs">
              <th class="px-4 py-2 bg-gray-200">S.No</th>
              <th class="px-4 py-2 bg-gray-200">Game Code</th>
              <th class="px-4 py-2 bg-gray-200">Game Name</th>
              <th class="px-4 py-2 bg-gray-200">Provider Name</th>
              <th class="px-4 py-2 bg-gray-200">Player</th>
              <th class="px-4 py-2 bg-gray-200">Round</th>
  ...
  return (
    ...
    setCurrentPage(1)
            ...
            >
              ...
              {providerData?.map((item) => {
                return <option value={item.gpId}>{item?.gpName}</option>;
              })}
            </Select>
          </div>
          <div className="flex w-[100%] flex-col  items-start ">
    ...
    setCurrentPage(1)
            ...
            <Select height={"30px"} style={{ border: `1px solid ${border}` }}>
              <option>INR</option>
        ...
        >
          ...
          <table className={`w-[100%]  `}>
            ...
            >
              ...
              <th className="min-w-[60px]">{t(`Game Name`)} </th>

              <th className="min-w-[50px]">{t(`Provider`)} {t(`Name`)}</th>
              <th className="min-w-[50px]">{t(`Player`)} {t(`Name`)}</th>
              <th className="min-w-[50px]">{t(`Round`)} </th>
              <th className="min-w-[50px]">{t(`Amount`)} </th>
...
export default ProfitLossByGame;
