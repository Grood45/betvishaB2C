...
const Casino = () => {
  ...
  console.log(providerCategory,"providerCarsd")
    ...
    <div className='mt-[150px]'>
      ...
      >
        <Flex className="tags-wrapper">
          <ul className="tags-list">
            <li className="active font-bold" style={{ color: secondaryText }} onClick={getAllCasinoData}>
              {t(`All`)}
            </li>
          </ul>
        </Flex>
      ...
      <div className=" ">
        <div className="tabs relative">
          {providers?.length > 0 && <div className="flex gap-2 px-3 tab-list">
            {providers?.length > 13&& (
                <button
            ...
            >
              <div
                ...
                className={`tab flex flex-col ${active == 0 ? "border-2 border-[#ffaa00]" : ""}  items-center `}
              ...
              {providers.map((provider) => (
                <div
                  key={provider._id}
                  className={`tab ${active == provider._id ? "border-2 border-[#ffaa00]" : ""} `}
                  style={{ backgroundColor: bgGray, ...TabsStyle }}
                  onClick={() => getFetchGameByProvider(provider)}
                >
                  <div className="flex flex-col items-center gap-2 w-[80px] h-[80px]">
                    <img
                      src={provider.image_url}
                      alt={provider.gpName}
                      className="w-[40px] h-10 object-cover rounded-[6px]"
                    />
                    <p className="mt-2 text-xs font-semibold text-center">{provider?.gpName?.slice(0, 10)}..</p>
                  </div>
                </div>
              ))}
            </div>
